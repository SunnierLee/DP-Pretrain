...
class DPOptimizer(Optimizer):
    ...
    def scale_grad(self):
        ...
        #print(self.real_cum_batch_size, self.expected_batch_size, self.accumulated_iterations)
        #self.real_cum_batch_size = 0
...
class CNDPOptimizer(Optimizer):
    ...
    def __init__(
        ...
        *,
        ...
        self.v = v
        self.gamma, self.beta, self.T = self.set_param_of_correlated_noise()
        self.history_noise = []

        for p in self.params:
            self.history_noise.append([])
            p.summed_grad = None
    
    def cal_beta(self, max_T, minimal=1e-25):
        import numpy as np
        beta = [1]
        ...
        res = np.array(beta)
        t = np.arange(max_T)
        beta = ((-1) ** t) * res * ((1 - self.v) ** t)
        # 防止后面在计算时发生数值下溢
        for i in range(len(beta)):
            if abs(beta[i]) <= minimal:
                break
        beta[i:] = -minimal
        return beta

    def set_param_of_correlated_noise(self, min_T=100, max_T=10000, interv=500, gamma_error=0.1):
        import numpy as np
        from scipy import integrate
        from scipy.linalg import toeplitz
        def integrated_func(w, T, beta):
            cos_ = np.cos(np.arange(T)*w)
            sin_ = np.sin(np.arange(T)*w)
            return 1/(np.sum(beta*cos_)**2 + np.sum(beta*sin_)**2)
        
        def beta2toeplitz(beta):
            # N = 2 * m + 1       # 总共的项数
            list2 = beta[::-1]  # 翻转
            t_matrix = toeplitz(beta, list2)
            return t_matrix

        def toeplitz2gamma(B):
            B_inv = np.linalg.pinv(B)
            gamma = []
            for j in range(B.shape[1]):
                gamma.append(np.linalg.norm(B_inv[:, j]))
            return np.max(gamma)
        
        beta_m = self.cal_beta(max_T)
        for T in range(min_T, max_T, interv):
            beta = beta_m[:T]
            B = beta2toeplitz(beta)
            gamma_real = toeplitz2gamma(B)
            gamma_asymptotic, _ = integrate.quad(integrated_func, -np.pi, np.pi, limit=500, args=(T, beta))
            gamma_asymptotic = np.sqrt(gamma_asymptotic/2/np.pi)
            print(gamma_real, gamma_asymptotic)
            if abs(gamma_real-gamma_asymptotic) <= gamma_error:
                break
        print("gamma error: {} T: {}".format(gamma_real-gamma_asymptotic, T))

        return gamma_real, list(beta), T
    
    def combine_noise(self, beta, noise, reference):
        beta = torch.tensor(beta[::-1]).view(1, -1).float()
        noise = torch.cat(noise).view(len(noise), -1)
        correlated_noise = torch.mm(beta.to(noise.device), noise).squeeze(0)
        return correlated_noise.view_as(reference)
    ...
    def add_noise(self):
        ...
        for p in self.params:
            _check_processed_flag(p.summed_grad)
            noise = _generate_noise(
                std=self.noise_multiplier * self.max_grad_norm * self.gamma,
                reference=p.summed_grad,
                generator=self.generator,
                secure_mode=self.secure_mode,
            )
            self.history_noise[c].append(noise.detach().unsqueeze(0))
            self.history_noise[c] = self.history_noise[c][-self.T:]
            p.grad = (p.summed_grad + self.combine_noise(beta=self.beta[:len(self.history_noise[c])], noise=self.history_noise[c], reference=p.summed_grad)).view_as(p)
            c += 1
